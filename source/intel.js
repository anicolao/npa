...
const PlayerNameIconRowLink = (player) => {
	...
	Crux.Text("", "section_title")
		.grid(6, 0, 21, 3)
		.rawHTML(`<a onclick="Crux.1crux.trigger('show_player_uid', '${player.uid}' )">${player.alias}</a>`)
		.roost(playerNameIconRow);
	...
	let loadHooks = function () {
		...
		Crux.format = function (s, templateData) {
			...
			while (fp >= 0 && i < 1000) {
				...
				} else if (sub.startsWith("https:&#x2F;&#x2F;i.imgur.com")) {
					s = s.replace(pattern,`<img style="width:100%;" src='${sub}' />`)
				} else {
