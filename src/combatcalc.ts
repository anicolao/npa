import { dist, Fleet, PlayerMap, ScanningData } from "./galaxy";
...
export const computeCombatOutcomes = (galaxy: ScanningData, staroutcomes?: { [k: string]: StarState }, maxTick?: number) => {
  ...
  for (const f in fleets) {
    ...
    if (fleet.o && fleet.o.length > 0) {
      ...
      if (maxTick !== undefined && galaxy.tick + ticks > maxTick)
        continue;
  ...
  for (const k in arrivals) {
    ...
    if (!starstate[starId]) {
      ...
      let totalDefense = stars[starId].st;
      const fleetStrength: {[k:string]: number} = {};
      ...
      for (const fleet of galaxy.stars[starId].fleetsInOrbit) {
        if (fleet.o.length > 0) {
          ...
          if (delay >= tick) {
            ...
            const origShips = starstate[starId].ships;
            if (departures[delay - 1] === undefined) {
              ...
              const leaving =
              starstate[starId].departures[tick].leaving + fleet.st;
              starstate[starId].departures[tick] = { leaving, origShips };
            }
            const leaving =
    ...
    if (tickDelta > 0) {
      ...
      if (starstate[starId].production) {
        ...
        starstate[starId].st +=
          Math.trunc(starstate[starId].production * tickDelta + oldc);
        starstate[starId].c =
    ...
    let defendersAggregate = starstate[starId].ships;
    while (offense > 0) {
