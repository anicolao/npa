...
function validateDiffCache(apikey: string) {
  function validateEntry(entry: any, i: number, skipCached?: boolean): void {
    ...
    if (entry.timestamp !== scanCacheEntry.timestamp) {
       if (i === 0) {
        console.log(`Expected Timestamp mismatch for ${i}: ${entry.timestamp} vs ${scanCacheEntry.timestamp}`, entry, scanCacheEntry)
       } else {
    ...
    if (entry.cached && !skipCached) {
      ...
      if (scanCachedOrComputed === undefined) {
        scanCachedOrComputed = window.structuredClone(scanCache[apikey][0].cached);
        for (let index = 0; index < i; ++index) {
          const forward = scanCache[apikey][index].forward;
          scanCachedOrComputed = patch(scanCachedOrComputed, forward);
        }
      }
      const nullDiff = diff(entry.cached, scanCachedOrComputed);
      if (nullDiff !== null) {
        console.error(`Index ${i} doesn't match on cached or computed `, nullDiff);
      } else {
...
export async function restoreFromDB(gameId: number, apikey: string) {
  if (!scanCache[apikey] || scanCache[apikey].length === 0) {
    try {
      ...
      validateDiffCache(apikey);
      //diffCache[apikey] = scanCache[apikey];
    } catch (err) {
      console.error(err);
    }
  }
...
export async function getServerScans(apikey: string) {
  ...
  console.log(`Reading diff database from time ${diffTimestamp}`)
  const unsubDiffs = onSnapshot(
    query(
      collection(firestore, diffskey),
      where("latest", ">", diffTimestamp),
      orderBy("latest"),
    ),
    (querySnapshot) => {
      ...
      changedBlocks.forEach((change, i) => {
        ...
        console.log(`Block ${i}: `, {i, latest: patches.latest, diffTimestamp, truth: patches.latest > diffTimestamp, startTick: JSON.parse(patches.initial_scan).scanning_data.tick}) ;
      });
      changedBlocks.forEach((change, i) => {
        ...
        const timestamps: number[] = Object.keys(patches).filter(x => +x > latestDiff).map(x => +x).sort();
        ...
        if (diffCache[apikey] === undefined || diffCache[apikey].length === 0) {
          ...
          diffCache[apikey] = [
            {
              cached, timestamp: patches?.initial_timestamp || cached.start_time
            }
          ];
        }
        timestamps.forEach(timestamp => {
          ...
          const origLast = diffCache[apikey].length - 1;
          let holeFound = false;
          ...
          while (scanCache[apikey].length > last) {
            ...
            diffCache[apikey].push({
              cached, back, timestamp: nextEntry.timestamp
            });
            if (last > 0) {
    ...
    if (entry.forward) {
      ...
      if (a != b) {
        console.error(`inproc Index ${apikey}:${last} doesn't match on forward`, { df: entry.forward, sf: scanCacheEntry.forward });
      } else {
